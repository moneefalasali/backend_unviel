...
class TextDetectionEngine implements DetectionEngineInterface
{
    ...
    public function analyze(array $input): array
    {
        ...
        $serviceResult = $this->aiService->analyzeText($text);
        $heuristicResult = $this->heuristic->analyzeText($text);
        ...
        $finalScore = $this->confidence->calculate([
            ...
            'ambiguous' => abs(($serviceResult['ai_percentage'] ?? 0) - ($heuristicResult['score'] ?? 0)) < 15,
        ]);

        $explain = $this->explainability->build('text', $serviceResult, $heuristicResult, $finalScore);
    ...
    private function buildExplanation(array $explain, array $serviceResult, array $heuristicResult): string
    {
        $parts = [
            $explain['explanation'],
            'Service output: ' . ($serviceResult['explanation'] ?? 'No service explanation provided.'),
            'Heuristic output: ' . ($heuristicResult['classification'] ?? 'No heuristics available.') . ' with score ' . ($heuristicResult['score'] ?? 0) . '%',
        ];
