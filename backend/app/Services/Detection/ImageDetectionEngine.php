...
class ImageDetectionEngine implements DetectionEngineInterface
{
    ...
    public function analyze(array $input): array
    {
        ...
        $serviceResult = $this->aiService->analyzeImage($image);
        $heuristicResult = $this->heuristic->analyzeImage($image);
        ...
        $explain = $this->explainability->build('image', $serviceResult, $heuristicResult, $finalScore);
    ...
    private function buildExplanation(array $explain, array $serviceResult, array $heuristicResult): string
    {
        return implode(' ', [
            ...
            'Heuristic score: ' . ($heuristicResult['score'] ?? 0) . '%.',
        ]);
    }
}
